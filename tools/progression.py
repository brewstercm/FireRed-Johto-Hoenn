...
def source_stage(version, location, area, method, species):
    ...
    rank = ranks[location]
    if version == 'crystal':
        # The western grass is reached through Union Cave with Surf.
        if location == 'ruins-of-alph' and method == 'walk' and species in ('natu', 'smeargle', 'quagsire'): rank = 4
        if location == 'dark-cave' and area == 'blackthorn-city-entrance': rank = 7
        if location == 'union-cave' and area == 'b2f': rank = max(rank, 4)
        if location == 'mt-mortar' and area == 'upper-cave': rank = 8
        if method.startswith('headbutt'): rank = max(rank, 2)
        if method == 'rock-smash': rank = max(rank, 3)
        if method in ('surf', 'good-rod'): rank = max(rank, 4)
        if method == 'super-rod': rank = max(rank, 9)
    else:
        if location == 'hoenn-route-111' and species in ('trapinch', 'cacnea', 'baltoy'): rank = 4
        if location == 'meteor-falls' and area in ('b1f', 'back', 'backsmall-room'): rank = 8
        if method == 'rock-smash': rank = max(rank, 3)
        if method in ('surf', 'good-rod', 'feebas-tile-fishing'): rank = max(rank, 5)
        if method in ('super-rod', 'seaweed'): rank = max(rank, 7)
    return rank
...
def target_stage(name):
    ...
    if route:
        n = int(route[1])
        for rank, nums in {0:(1,2,22), 1:(3,4), 2:(5,6,24,25), 3:(9,10,11),
                           4:(7,8,12,13,14,15,16,17,18), 5:(19,20,21), 8:(23,)}.items():
            if n in nums: return rank
    prefixes = {
        0: ('VIRIDIAN_FOREST', 'PALLET_TOWN', 'VIRIDIAN_CITY'),
        1: ('MT_MOON',), 2: ('CERULEAN_CITY', 'VERMILION_CITY', 'SSANNE', 'DIGLETTS_CAVE'),
        3: ('ROCK_TUNNEL',), 4: ('POKEMON_TOWER', 'CELADON_CITY', 'FUCHSIA_CITY', 'SAFARI_ZONE'),
    ...
    for rank, names in prefixes.items():
        if name.startswith(names): return rank
    raise ValueError('Unranked FireRed map: ' + name)

def habitat(name):
    name = name.lower()
    if any(s in name for s in ('seafoam', 'shoal', 'ice-path', 'icefall')): return 'ice'
    if any(s in name for s in ('pyre', 'tower_', 'pokemon-tower', 'lost-cave', 'mt-silver')): return 'ghost'
    if any(s in name for s in ('fiery', 'jagged', 'magma', 'ember', 'mansion')): return 'fire'
    if any(s in name for s in ('cave', 'moon', 'tunnel', 'mortar', 'meteor', 'victory', 'granite', 'ruins', 'mirage-tower')): return 'cave'
    if any(s in name for s in ('forest', 'woods', 'park', 'bush')): return 'forest'
    return 'field'
...
def target_phase(rank):
    # FireRed opens its late-game habitats with Surf after Koga, whereas
    # Johto/Hoenn spread those habitats across more badges.
