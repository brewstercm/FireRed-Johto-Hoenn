...
import build
from progression import source_stage, source_phase, target_phase
...
assert {p['dex'] for p in placements} | {p['dex'] for p in excluded} == set(range(152,387))
...
for p in placements:
    assert target_phase(p['target_stage']) >= source_phase(p['source_stage'])
    assert 1 <= p['min_level'] <= p['max_level'] <= 100
    assert not p['evolved'] or p['min_level'] >= p['source_min']
    mass[p['map'],p['terrain']] += p['weight']
assert max(mass.values()) <= 40

species = {name:int(num) for name,num in re.findall(r'^#define SPECIES_(\w+) (\d+)\s*$',
    (ROOT/'tools/source/firered_species.h').read_text(),re.M)}
assert species['RALTS'] == 392 # National Dex 280 is NOT the native species ID.
ids = build.map_ids()
source_tables = json.loads((ROOT/'tools/source/firered_wild_encounters.json').read_text())['wild_encounter_groups'][0]['encounters']

def run(runtime_name):
    lua = importlib.import_module('lupa.'+runtime_name).LuaRuntime(unpack_returned_tuples=True)
    def table(v):
    ...
    lua.execute('package.loaded["src.mods.Merge"] = assert(load(read_file("tools/reference/Merge.lua")))()')
    lua.execute('Schemas = assert(load(read_file("tools/reference/release_Schemas.lua")))(); Merge = require("src.mods.Merge")')
    lua.execute('package.loaded["src.import.gba.versions"] = assert(load(read_file("tools/reference/versions.lua")))()')
    lua.execute('package.loaded["src.import.gba.map_groups_firered"] = assert(load(read_file("tools/reference/src_import_gba_map_groups_firered.lua")))()')
    lua.execute('Catalog = assert(load(read_file("tools/reference/map_catalog.lua")))()')
    # Validate generated IDs against the actual engine's canonical map catalog.
    for p in placements: assert lua.eval('Catalog.isKnown')(p['map']), p['map']
    native={}
    field_map={'land_mons':'land','water_mons':'water','fishing_mons':'fishing','rock_smash_mons':'rocks'}
    for row in source_tables:
        name=row['map'][4:]
        if not row['base_label'].endswith('_FireRed') or name not in ids: continue
        map_id=ids[name]
        ...
        for field,terrain in field_map.items():
            if field in row:
                out[terrain]={'rate':row[field]['encounter_rate'],'slots':[
                    {'species':species[s['species'][8:]],'minLevel':s['min_level'],'maxLevel':s['max_level']}
                    for s in row[field]['mons']]}
        native[map_id]=out
        native[f'{group}:{num}']=out
    lua.globals().native=table(native)
    lua.globals().names=table({num:name for name,num in species.items() if num>0})
    lua.execute('''
    ...
    expected={(p['map'],p['field'],p['slot']):species[p['species']] for p in placements}
    for map_id in {p['map'] for p in placements}:
        before=g.before[map_id]; after=g.native[map_id]
        for terrain in ('land','water','fishing','rocks'):
            if before[terrain] is None: assert after[terrain] is None; continue
            assert before[terrain]['rate']==after[terrain]['rate']
            a,b=before[terrain]['slots'],after[terrain]['slots']
            assert len(a)==len(b)
            for i in range(1,len(a)+1):
                assert a[i]['minLevel']==b[i]['minLevel'] and a[i]['maxLevel']==b[i]['maxLevel']
                assert b[i]['species']==expected.get((map_id,terrain,i),a[i]['species'])
            ranges=((1,2),(3,5),(6,10)) if terrain=='fishing' else ((1,len(a)),)
            for lo,hi in ranges:
                assert {a[i]['species'] for i in range(lo,hi+1)} <= {b[i]['species'] for i in range(lo,hi+1)}
    ...
    ''')
    print(runtime_name+': 118 native species IDs, real schema/merge/write, canonical maps, aliases, levels/rates, native preservation, conflict/missing-data checks PASS')

for runtime in ('lua54','luajit21'): run(runtime)
print('Progression, coverage, evolved-form level floors, <=40% replacement mass: PASS')
